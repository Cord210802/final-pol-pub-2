...
El primer componente es una secuencia de modelos de probabilidad lineal jerárquicos. Aunque una opción común para variables dependientes binarias es la regresión logística, los análisis implementados estiman modelos lineales por mínimos cuadrados ordinarios con errores estándar robustos HC1. Por esa razón, esta sección describe el enfoque efectivamente estimado. Su ventaja práctica es que los coeficientes se interpretan directamente como cambios en probabilidad, medidos en puntos porcentuales; su limitación es que no impone que las predicciones estén siempre dentro del intervalo $[0,1]$.

Para cada persona $i$, el modelo general es
$$
Y_i = \alpha_m + \sum_{b=1}^{m} \mathbf{X}_{ib}'\boldsymbol{\beta}_b + \varepsilon_i,
$$
donde $Y_i$ es `ASSAULT_ADULT`, $\mathbf{X}_{ib}$ es el vector de variables del bloque $b$, $\boldsymbol{\beta}_b$ es el vector de coeficientes asociado a ese bloque, $\alpha_m$ es la constante del modelo $m$, y $\varepsilon_i$ es el término de error. Todos los modelos se estiman sobre la misma muestra analítica, definida por eliminación por lista en la variable dependiente y en los 26 predictores usados por el modelo completo. Esto permite que los cambios de ajuste entre modelos reflejen la contribución del bloque agregado y no cambios en la composición de la muestra.
...
\end{aligned}
$$
En esta notación, $D$ corresponde al bloque demográfico, $F$ al bloque familiar y socioeconómico, $C$ al bloque cognitivo, $B$ al bloque de conducta pasada, y $J$ al bloque de justicia y sustancias. El modelo $M_5$ es el modelo lineal principal porque contiene todos los predictores preadultos definidos en el análisis. También se estima $M_6$, que añade dos interacciones entre sexo y raza o etnicidad, pero ese modelo se trata como extensión.
...
\frac{(RSS_r - RSS_a)/(df_r - df_a)}
     {RSS_a/df_a},
$$
donde $RSS_r$ y $RSS_a$ son las sumas de cuadrados residuales del modelo reducido y del modelo amplio, respectivamente, y $df_r - df_a$ es el número de predictores añadidos. La hipótesis nula es que todos los coeficientes del bloque agregado son iguales a cero. Además, se reportan $R^2$, $R^2$ ajustada, AIC y BIC. La evaluación fuera de muestra se realiza únicamente para el modelo lineal completo $M_5$: la muestra se divide en 85 por ciento de entrenamiento y 15 por ciento de prueba, con estratificación por $Y_i$, y se reportan $R^2$, RMSE, MAE, accuracy con umbral 0.5 y AUC. El AUC recibe más peso interpretativo porque resume la capacidad de ordenar correctamente casos positivos y negativos sin depender de un umbral fijo.
